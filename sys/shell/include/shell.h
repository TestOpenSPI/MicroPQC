...
#define SHELL_IN_SECTION _Pragma("location=\"shell_command\"")
#define DEFINE_SHELL_CMD(_cmd,_desc,_handler) \
   SHELL_IN_SECTION const shell_command_t _cmd = { .name = #_cmd, .desc = _desc, .handler = _handler }
#else
#define DEFINE_SHELL_CMD(_cmd,_desc,_handler) \
	const shell_command_t _cmd __attribute__((used,section("shell_command"))) = { .name = #_cmd, .desc = _desc, .handler = _handler }
#endif
...
/**
 * @brief           shell thread
 * @details         OS should launch this thread to use shell
 * @param[in]       arg will be casted by (const shell_command_t *)
 */
